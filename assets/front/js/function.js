...
function setHeaderHeight(){
	...
	/* trainers-and-studio */
	if ($('.trainers-and-studio').length) {
		const testimonial_slider = new Swiper('.trainers-and-studio .swiper', {
			slidesPerView : 1,
			speed: 1000,
			spaceBetween: 30,
			loop: true,
	...
	function submitForm(){
		...
		$.ajax({
			...
			beforeSend:function(){
				...
				$('#contactForm').find('button[type="submit"]').text('Please Wait...');
			},
			...
			complete:function(){
				...
				$('#contactForm').find('button[type="submit"]').text('send message');
			}
		});
	}
	...
	function submitappointmentForm(){
		...
		$.ajax({
			type: "POST",
			url: base_url + 'book-apointment',
			data: "name=" + name + "&email=" + email + "&phone=" + phone + "&services=" + services + "&msg=" + msg,
			beforeSend:function(){
				$('#appointmentForm').find('button[type="submit"]').prop('disabled', true);
				$('#appointmentForm').find('button[type="submit"]').text('Please Wait...');
			},
			success : function(text){
				if (text == "success"){
			...
			complete:function(){
				...
				$('#appointmentForm').find('button[type="submit"]').text('send message');
			}
		});
	}
