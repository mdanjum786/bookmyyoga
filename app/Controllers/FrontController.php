...
class FrontController extends MyController
{
    ...
    public function pincode_autocomplete() {
        $q = $this->request->getGet('q');
        $database = \Config\Database::connect();
        $builder = $database->table('pincodes as p');
        $builder->select('p.id, p.pincode, p.region_name, p.city_id, c.name as city_name, c.state_id, s.name as state_name, s.country_id, co.name as country_name');
        $builder->join('city as c', 'p.city_id = c.id', 'left');
        $builder->join('state as s', 'c.state_id = s.id', 'left');
        $builder->join('country as co', 's.country_id = co.id', 'left');
        if ($q) {
            $builder->like('p.pincode', $q);
            $builder->orLike('p.region_name', $q);
        }
...
";
   ...
   public function videoGallery(){
    ...
    //         ],
    //    $rules = [
    //         "email" => ["rules" => "required|valid_email|is_unique[members.email]"],
    //         "password" => ["rules" => "required|min_length[6]"],
    //         "name" => ["rules" => 'required'],
    //         "mobile_no" => ["rules" => 'required|min_length[10]'],
    //         "aadhar_no" => ["rules" => 'required|min_length[12]'],
    //         "city" => ["rules" => 'required'],
    //         "nationality" => ["rules" => 'required'],
    //         "gender" => ["rules" => 'required'],
    //         "address" => ["rules" => 'required'],
    //         "pincode" => ["rules" => 'required'],
    //         'profession' => ["rules" => 'required'],
    //         'profile' => ['rules'  => 'uploaded[profile]|mime_in[profile,image/jpeg,image/png,image/jpg]']
    //     ];
    //             "label" => "Pincode",
    //             "rules" => 'required'
    //         ],
    ...
    public function trainers_and_studio(){
        ...
        // Always load all states for the dropdown
        $allStates = $database->table('state')->where('status', 1)->get()->getResultArray();
        $data['state'] = $allStates;
        $data['city'] = [];
        $data['pincode'] = '';
        if($trainers_and_studio_data && isset($trainers_and_studio_data['pincode_id'])){
            // Get pincode details
            $pincodeRow = $database->table('pincodes')->where('id', $trainers_and_studio_data['pincode_id'])->get()->getRowArray();
            if($pincodeRow){
                $data['pincode'] = $pincodeRow['pincode'];
                // Get city, state, country from pincode
                $cityRow = $database->table('city')->where('id', $pincodeRow['city_id'])->get()->getRowArray();
                if($cityRow){
                    $data['city'] = [$cityRow];
                    $stateRow = $database->table('state')->where('id', $cityRow['state_id'])->get()->getRowArray();
                    if($stateRow){
                        // Populate all cities for this state for dropdown
                        $cityList = $database->table('city')->where('state_id', $stateRow['id'])->where('status', 1)->get()->getResultArray();
                        $data['city_dropdown'] = $cityList;
                        $countryRow = $database->table('country')->where('id', $stateRow['country_id'])->get()->getRowArray();
                        if($countryRow){
                            $data['countries'] = [$countryRow];
                        } else {
    ...
    public function update_trainer_studio(){
        ...
        $database = \Config\Database::connect();
        $manualPincode = $this->request->getPost('manual_pincode');
        $manualRegion = $this->request->getPost('manual_region');
        $pincodeId = null;
        $manualCityId = null;

        // If manual city is entered
        if(null !== $this->request->getPost('other_city_checkbox')){
            // Add new city if needed
            $dataCity = [
                'state_id' => $this->request->getPost('state_id'),
                'name' => $this->request->getPost('other_city'),
                'status' => 1
            ];
            $database->table('city')->insert($dataCity);
            $manualCityId = $database->insertID();
            $updateData['city_id'] = $manualCityId;
        }

        if ($manualPincode && $manualRegion) {
            // Check if pincode+region exists
            $cityIdForPincode = $manualCityId ? $manualCityId : $this->request->getPost('city_id');
            $pincodeRow = $database->table('pincodes')
                ->where('pincode', $manualPincode)
                ->where('region_name', $manualRegion)
                ->get()->getRowArray();
            if (!$pincodeRow) {
                // Insert new pincode+region
                $insertData = [
                    'pincode' => $manualPincode,
                    'region_name' => $manualRegion,
                    'city_id' => $cityIdForPincode
                ];
                $database->table('pincodes')->insert($insertData);
                $pincodeId = $database->insertID();
            } else {
                $pincodeId = $pincodeRow['id'];
            }
            $updateData['pincode_id'] = $pincodeId;
        } else {
            // Use selected pincode if not manual
            $pincode = $this->request->getPost('pincode');
            $pincodeRow = $database->table('pincodes')->where('pincode', $pincode)->get()->getRowArray();
            if($pincodeRow){
                $pincodeId = $pincodeRow['id'];
                $updateData['pincode_id'] = $pincodeId;
                $updateData['city_id'] = $pincodeRow['city_id'];
                // Get city, state, country
                $cityRow = $database->table('city')->where('id', $pincodeRow['city_id'])->get()->getRowArray();
                if($cityRow){
                    $updateData['state_id'] = $cityRow['state_id'];
                    $stateRow = $database->table('state')->where('id', $cityRow['state_id'])->get()->getRowArray();
                    if($stateRow){
                        $updateData['country_id'] = $stateRow['country_id'];
                    }
                }
            } else {
                // fallback to manual city/state/country selection
                $updateData['country_id'] = $this->request->getPost('country_id');
                $updateData['state_id'] = $this->request->getPost('state_id');
                $updateData['city_id'] = $manualCityId ? $manualCityId : $this->request->getPost('city_id');
            }
        }
        if ($pincodeId) {
            $updateData['pincode_id'] = $pincodeId;
        ...
        if($user){
            ...
            if($banner_image && $banner_image->getName() != ""){
    ...
    public function trainer_studio_list(){
       ...
       // $perPage=12;
        ...
        $builder->join('city as c', 't1.city_id = c.id', 'left');
        $builder->join('pincodes as p', 'p.city_id = t1.city_id', 'left');
        $builder->select('t1.*, c.name as cname, p.pincode, p.region_name');
        $builder->where('t1.status',1);

        // Optionally join state if needed for search
        $builder->join('state as s', 't1.state_id = s.id', 'left');

        if ($q) {
            $builder->groupStart();
            $builder->like('t1.business_name', $q)
                    ->orLike('c.name', $q)
                    ->orLike('p.pincode', $q)
                    ->orLike('p.region_name', $q)
                    ->orLike('s.name', $q);
            $builder->groupEnd();
        }
            if($orderby == 'random'){
        ...
        $data2 = $builder->get($perPage,$offset)->getResultArray();

        // For pagination, count total with same filters
        $builder_count = $database->table('trainers_and_studio as t1');
        $builder_count->join('city as c', 't1.city_id = c.id', 'left');
        $builder_count->join('pincodes as p', 'p.city_id = t1.city_id', 'left');
        $builder_count->join('state as s', 't1.state_id = s.id', 'left');
        $builder_count->select('t1.id');
        $builder_count->where('t1.status',1);
        if ($q) {
            $builder_count->groupStart();
            $builder_count->like('t1.business_name', $q)
                    ->orLike('c.name', $q)
                    ->orLike('p.pincode', $q)
                    ->orLike('p.region_name', $q)
                    ->orLike('s.name', $q)
                    ->orLike('t1.address', $q)
                    ->orLike('t1.about_us', $q);
            $builder_count->groupEnd();
        }
        $total = $builder_count->countAllResults();
        ...
        // Pincode autocomplete for trainer studio form
    public function pincode_autocomplete2()
    {
        $q = $this->request->getGet('q');
        $database = \Config\Database::connect();
        $builder = $database->table('pincodes as p');
