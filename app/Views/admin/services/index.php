...
<table class="table table-striped " id="services-table">
	...
	<tbody>
		...
		<tr>
			<td><?= $i++; ?></td>
			<td><?= $event['title'] ?></td>
			<td><?= (int)$event['price'] ?></td>
			<td><?= (int)$event['offer_price'] ?></td>
			<td><img src="<?= base_url('assets/services/') . $event['image'] ?>" width="150"></td>
			
			<td><?= $event['type'] ?></td>
			<td><?= $event['short_description'] ?></td>
			<td><?= $event['status'] == 1 ? "Active" : "In active" ?></td>
			<td>
				<a href="<?= base_url('admin/services/edit/'). $event['id']?>" class="" data-id="<?= $event['id'] ?>"><i class="fas fa-edit"></i></a>
				<br>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       <form class="form" id="edit-event-form">
		...
		<input type="hidden" name="additional_image2">
	</form>
      </div>
...
<script type="text/javascript">
	$(function(){
		...
		$(document).on('submit', '#change-status-form', function(e){
			...
			$.ajax({
				type:'POST',
				url:"<?= base_url('admin/services/change-status')?>",
		...
		$(document).on('click','.edit',function(e){
			...
			$.ajax({
				type : 'POST',
				url: "<?= base_url('admin/services/get-event-data')?>",
				data:{id:_id},
				dataType:'json',
				beforeSend:function(){
				...
				success:function(res){
					...
					$('#edit-event-form').find('#event-image').attr('src', "<?=base_url('assets/services/')?>" + res.image);

					let urls = "<?=base_url('assets/services/')?>";
					if(res.additional_image != ""){
						$('#edit-event-form').find('.additional_image-container').html(`<img width="200" src="${urls}${ res.additional_image}" />`)
					}else{
						$('#edit-event-form').find('.additional_image-container').html(``)
					}
