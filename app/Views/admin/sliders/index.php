...
<div style="float: right;">
		<a href="<?php echo base_url('admin/slider/create');?>" class="btn btn-info">Add Slider</a>
	</div>
<h3>Slider List</h3>
<hr>
	<table class="table table-striped" id="gallery-list">
		...
		<tbody>
			...
			<tr>
				<td><?= $i++; ?></td>
				<td><?= $image['heading']; ?></td>
				<td><?= $image['subheading']; ?></td>
				<td><?= $image['slider_btn_text']; ?></td>
				<td><?= $image['slider_btn_link']; ?></td>
				<td><img src="<?= base_url('assets/sliders/') . $image['image'];?>" width="100"></td>
				<td><?= $status; ?></td>
				<td>
					<a href="<?= base_url('admin/slider/edit/') . $image['id']?>" class="editww" data-id="<?= $image['id']?>"  data-status="<?= $image['status']; ?>"><i class="fas fa-edit"></i></a>
					<br />
	...
	<script type="text/javascript">
		$(function(){
			...
			$(document).on('submit', '#gallery-status-form', function(e){
				...
				$.ajax({
					type: 'POST',
					url : "<?= base_url('admin/slider/editSlider')?>",
